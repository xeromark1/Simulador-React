...
export function ShowSeccionSelected(n){ //funcion que es creada para agragar los ramos a la tabla
    ...
    if( 0 < n && n <= 5){
        let s = n + 1;
        var ComboBoxRamoSiguiente = document.getElementById("ComboBoxRamo"+s);
        var ComboBoxSeccionSiguiente = document.getElementById("ComboBoxSeccion"+s);
        ComboBoxRamoSiguiente.disabled = false;
        ComboBoxSeccionSiguiente.disabled = false;
    }
    ...
    if(n == 6){ console.log("codigo"+n);}
        ...
        sessionStorage.setItem("codigo"+n, JSON.stringify(auxiliar) );       //OJO ACA, Esto define si se podra quitar o no
            ...
            aplicarTodosLosCombobox(n);  //esta funcion lo que hace es que al seleccionar una seccion, en todos los combobox se deben actualizar para tomar ramos
        ...
        if(i != 1){
            ...
            <div>
                    <div id = {"DivRamo"+i}>
                            ...
                            <select id ={"ComboBoxRamo"+i} onChange= {(e) => {ShowRamoSelected(i)}} disabled> <option disabled selected>Seleccione ramo </option>
                            {
                                ramos.map( ramo =>{
                    ...
                    <div id = {"DivSeccion"+i}>
                            ...
                            <select id ={"ComboBoxSeccion"+i} onChange= {(e) => {ShowSeccionSelected(i)}} disabled> </select>
...
export function aplicarTodosLosCombobox(n){
    ...
    for(let i = 1 ; i < 7 ; i++){
        ...
        if(i != n &&  SelectionComboBoxSeccion != "Seccion" &&  SelectionComboBoxSeccion != ""){

            const Lista = [];
            const aux = [];         // es un auxiliar para saber luego si la seccion fue agregada
            var ComboBoxRamo = document.getElementById('ComboBoxRamo'+i).value;
            ...
            for(let seccion of datos){        //El codigo que se presenta a continuacion es para ir bloqueando las secciones que no nos sirven porque estan siendo obstruidas por los ramos anteriores
                if(seccion.id == ComboBoxRamo){
...
//--- Ahora colocamos todas las secciones en la ComboBox
            Lista.push(
                secciones.map( seccion =>{
                    if(aux.indexOf(seccion.codigo) == -1 && seccion.id == ComboBoxRamo && SelectionComboBoxSeccion != "" && SelectionComboBoxSeccion != null){    {/*Aca se agregan las secciones*/}
                        
                        if(SelectionComboBoxSeccion == seccion.codigo){     // Si la seccion ya estaba seleccionada antes, entonces que la deje seleccionada
                            aux.push(seccion.codigo);
                            return <option value = {seccion.codigo} selected>{seccion.codigo}</option>
                        }
                        else if(choqueDeDias.indexOf(seccion.codigo) == -1){      // si el ramo NO esta, es porque NO obstruye las selecciones anteriores 
                            aux.push(seccion.codigo);
                            return <option value = {seccion.codigo}>{seccion.codigo}</option>
                        }
                        else if (choqueDeDias.indexOf(seccion.codigo) != -1){// si el ramo esta, es porque esta siendo obstruido por sus selecciones anteriores
                            aux.push(seccion.codigo);
                            return <option value = {seccion.codigo} disabled>{seccion.codigo}</option>
                        }
                    } 
            ...
            const ComboBoxSeccion = ReactDOM.createRoot( document.getElementById('ComboBoxSeccion'+i));

            console.log("ComboBoxSeccion");
